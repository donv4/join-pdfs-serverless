...
class PDFChatWidget {
    ...
    init() {
        this.createWidget();
        this.bindEvents();
        console.log('PDF Chat Widget initialized with Serverless Route');
    }
    
    createWidget() {
        this.container = document.createElement('div');
        this.container.id = 'pdf-chat-widget';
        this.container.innerHTML = `
            <div class="chat-widget-toggle" id="chatToggle" style="position: fixed; bottom: 20px; right: 20px; width: 60px; height: 60px; background: linear-gradient(135deg, #4a6cf7, #667eea); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px; cursor: pointer; box-shadow: 0 4px 15px rgba(0,0,0,0.15); z-index: 999999; transition: transform 0.2s;">
                <span class="chat-icon">🤖</span>
            </div>
            <div class="chat-widget-container" id="chatContainer" style="display: none; position: fixed; bottom: 20px; right: 20px; width: 350px; height: 450px; background: white; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15); border: 1px solid #e2e8f0; z-index: 999999; overflow: hidden; font-family: system-ui, sans-serif;">
                <div class="chat-header" style="background: linear-gradient(135deg, #4a6cf7, #667eea); color: white; padding: 15px; display: flex; justify-content: space-between; align-items: center; font-weight: bold;">
                    ...
                    <button class="close-chat" id="closeChat" style="background: transparent; border: none; color: white; font-size: 24px; cursor: pointer; line-height: 1;">×</button>
                </div>
                <div class="chat-messages" id="chatMessages" style="height: 290px; overflow-y: auto; padding: 15px; background: #f8fafc; display: flex; flex-direction: column; gap: 10px; text-align: left;">
                ...
                <div class="chat-input" style="padding: 10px; background: white; border-top: 1px solid #e2e8f0; display: flex; gap: 8px;">
                    <input type="text" id="chatInput" placeholder="Type your message..." style="flex: 1; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 14px; outline: none;" />
                    <button id="sendMessage" style="background: #4a6cf7; color: white; border: none; padding: 10px 16px; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 14px;">Send</button>
                </div>
    ...
    async sendMessage() {
        ...
        try {
            // 🔒 Re-routed to your central serverless chatbot worker API path
